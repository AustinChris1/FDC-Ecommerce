...
import trackImg from '../assets/Track-your-order.webp';
...
const TrackOrder = () => {
    ...
    // Helper function to format customer name safely - using full name for clarity
    ...
    const fetchOrderTracking = async (e) => {
        ...
        try {
            ...
            if (res.data.status === 200 && res.data.order) {
                ...
                // Add Processing if applicable (only if not yet shipped)
                const isProcessingStatus = order.status.includes('processing') || order.status.includes('pending_');
                ...
                if (order.shipped_at) {
                    trackingHistory.push({
                        ...
                        location: order.origin_location || order.city || 'Logistics Hub',
                ...
                if (order.out_for_delivery_at) {
                    trackingHistory.push({
                        ...
                        location: order.delivery_route_info || order.city,
                ...
                if (order.delivered_at) {
                    trackingHistory.push({
                        ...
                        location: order.shipping_address1 ? `${order.shipping_address1}, ${order.city}, ${order.state}` : 'Customer Address',
                ...
                // Add Cancelled/Failed if cancelled_at is present
                ...
                // Re-sort after potential replacements
                uniqueTrackingHistory.sort((a, b) => new Date(a.date) - new Date(b.date));

                setOrderTrackingData({
                    ...order,
                    trackingHistory: uniqueTrackingHistory,
                    items_json: Array.isArray(order.items_json) ? order.items_json : (order.items_json ? JSON.parse(order.items_json) : [])
                });
    ...
    useEffect(() => {
        document.title = "Track Your Order - FirstSmart Mart";
    }, []);
    ...
    const getTrackingEventIcon = (iconName, colorClass) => {
        const IconComponent = {
            Calendar: Calendar,
            Clock: Clock,
            Truck: Truck,
        ...
        return IconComponent ? <IconComponent className={`w-5 h-5 ${colorClass}`} /> : <Package className="w-5 h-5 text-gray-500 dark:text-gray-400" />;
    };

    return (
        <motion.div
            className="min-h-screen p-4 sm:p-6 lg:p-8 pt-24
        ...
        >
            <Helmet>
                <title>Track Your Order - FirstSmart Mart</title>
                <meta name="description" content="Get real-time updates on your FirstSmart Mart order. Enter your order number for detailed tracking information." />
            </Helmet>
            ...
            <header className="flex mt-20 flex-col sm:flex-row justify-between items-start sm:items-center mb-8
                ...
                <form onSubmit={fetchOrderTracking} className="flex flex-col sm:flex-row w-full sm:w-auto space-y-3 sm:space-y-0 sm:space-x-3">
                    ...
                    <button
                        type="button"
                        onClick={() => fetchOrderTracking({ preventDefault: () => { } })}
                        disabled={loading}
                        className="px-4 py-3 bg-gray-200 hover:bg-gray-300 text-gray-800 font-semibold rounded-lg shadow-md
                                   transition-all duration-300 transform hover:scale-105 flex items-center justify-center
                                   disabled:opacity-50 disabled:cursor-not-allowed
            ...
            <AnimatePresence mode="wait">
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="space-y-3 text-gray-700 dark:text-gray-300">
                                ...
                                {/* NEW: POS Sale Status */}
                                <p className="flex justify-between items-center">
                                    <span className="font-semibold text-gray-600 dark:text-gray-400">Sale Type:</span>
                                    <span className={`font-semibold ${orderTrackingData.is_pos_sale ? 'text-purple-600 dark:text-purple-400' : 'text-blue-600 dark:text-blue-400'}`}>
                                ...
                                {orderTrackingData.is_pos_sale && orderTrackingData.location_name && (
                                    <p className="flex justify-between items-center">
                                        ...
                                        <span className="flex items-center text-gray-700 dark:text-gray-300">
                                            ...
                                            {orderTrackingData.location_name}
                                            {orderTrackingData.location_address && `, ${orderTrackingData.location_address}`}
                                        </span>
                                    </p>
                                )}
                                ...
                                <p className="flex justify-between items-center">
                                    ...
                                    <span className="text-gray-700 dark:text-gray-300">{formatCustomerName(orderTrackingData.full_name)}</span>
                                </p>
                                    ...
                                    orderTrackingData.trackingHistory.map((event, index) => (
                                        <div key={index} className="relative mb-8 last:mb-0">
                                            <div className="absolute -left-3 top-0 flex items-center justify-center w-6 h-6 rounded-full bg-blue-100 dark:bg-blue-600 text-blue-600 dark:text-white">
                                                {getTrackingEventIcon(event.icon, `text-${event.color}-500 dark:text-${event.color}-400`)}
                                            </div>
                                            <div className="ml-4">
                        ...
                        {orderTrackingData.items_json && Array.isArray(orderTrackingData.items_json) && orderTrackingData.items_json.length > 0 && (
                            <motion.div
                                ...
                                <div className="overflow-x-auto">
                                    <table className="min-w-full divide-y divide-gray-300 dark:divide-gray-700">
                                        ...
                                        <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                            {orderTrackingData.items_json.map((item, index) => (
                ...
                {!loading && !orderTrackingData && !error && (
                    ...
                    >
                        <img
                            src={trackImg}
                            alt="A person tracking a package on a mobile phone"
                            className="mx-auto mb-6 rounded-lg shadow-md"
                        />
                        <h2 className="text-3xl font-bold text-gray-800 mb-3 dark:text-white">Track Your Order</h2>
